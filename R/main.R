# Author: Edoardo Filippi 
# mail: efilippi@uni-mainz.de


source("R/init.r")

main("C:/Users/Edoardo/Desktop/Seurat/Organized_data/_pipeline_files/microglia.json")
main("C:/Users/Edoardo/Desktop/Seurat/Organized_data/_pipeline_files/microglia_bordermacrophages.json")

main("C:/Users/Edoardo/Desktop/Seurat/Organized_data/_pipeline_files/wgcna_microglia_all.json")
main("C:/Users/Edoardo/Desktop/Seurat/Organized_data/_pipeline_files/wgcna_microglia_mri_subset.json")

main("C:/Users/Edoardo/Desktop/Seurat/Organized_data/_pipeline_files/cellchat_microglia.json")
main("C:/Users/Edoardo/Desktop/Seurat/Organized_data/_pipeline_files/cellchat_microglia_astrocytes.json")
main("C:/Users/Edoardo/Desktop/Seurat/Organized_data/_pipeline_files/cellchat_all.json")

main("C:/Users/Edoardo/Desktop/Seurat/Organized_data/_pipeline_files/all_main.json")
# main("from_beginning.json")

# main("from_beginning_s.json")

# main("pipeline_complete_all.json")

# main("pipeline_complete_microglia_from_old.json")

# main("pipeline_complete_microglia_selection.json")


# "significance_membership_scatter"
# "significance_membership_scatter"
# "significance_membership_scatter"
# "significance_membership_model"
# "heatmap_pathology", 
# "heatmap_mri", 
# "heatmap_zscore", 

# "significance_membership_scatter"
# "significance_membership_model"
# "heatmap_pathology", 
# "heatmap_mri", 
# "heatmap_zscore", 

# "violin_plots", 
# "histogram_plot", 
# "histogram_plot_significance",  
# "significance_log2fc_scatter", 
# "correlation_avglog2fc_scatter", 
# "significance_membership_scatter",
# "significance_membership_model",
# "heatmap_pathology", 
# "heatmap_mri",
# "heatmap_zscore" ],  

# "violin_plots",  
# "histogram_plot",
# "heatmap_pathology"]

# "violin_plots", 
# "histogram_plot", 
# "histogram_plot_significance",  
# "heatmap_pathology", 
# "corr_matrix" ],     